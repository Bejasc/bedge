...
- [x] `CLAUDE.md` documents test location, naming convention, and available `pnpm` commands
- [x] `pnpm test` from the repo root runs all workspace tests and exits 0
- [x] `pnpm --filter bot test` runs bot tests independently
- [x] `pnpm --filter api test` runs api tests independently
- [x] At least one passing test exists in `apps/bot/tests/`
- [x] At least one passing test exists in `apps/api/tests/`
- [x] `.claude/commands/draft-pr.md` exists and selects the correct `type:*` label from the standard set
- [x] `CLAUDE.md` references `/draft-pr` with no dead links
